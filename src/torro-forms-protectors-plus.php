 ...
 * Text Domain: torro-forms-protections-plus
 ...
 * have to call `torro()->extensions()->get( 'torro_forms_protections_plus' )`.
 *
 * @since 1.0.0
 *
 ...
 */
function torro_forms_protections_plus_load( $torro ) {
	...
	$class_name = 'awsmug\TorroFormsProtectionsPlus\Extension';
	...
	if ( preg_match( '#^' . preg_quote( $mu_plugin_dir, '#' ) . '/#', $file ) && file_exists( $mu_plugin_dir . '/torro-forms-protections-plus.php' ) ) {
		$basedir_relative = 'torro-forms-protections-plus/';
	}

	$result = $torro->extensions()->register( 'torro_forms_protections_plus', $class_name, $main_file, $basedir_relative );
...
if ( function_exists( 'torro_load' ) ) {
	torro_load( 'torro_forms_protections_plus_load' );
} else {
	add_action( 'torro_loaded', 'torro_forms_protections_plus_load', 10, 1 );
}
